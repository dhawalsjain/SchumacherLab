...
if(F){
  ...
  save(res,file=paste0(DATADIR,"33_DRE_DE_matrix.RData"))
...
if(F){
  ...
  save(out,out1,file=paste0(DATADIR,"33_DRE_DE_GOEnrichment.RData"))
...
if(T){
  ...
  out <- c()
  for(feat in c("3_prime_UTR","intron")  ){ levels(as.factor(res$snpeff_uq))
    for(f in c("up","dn")){
      cat(feat,"\t",f,"..\t")
      if(f == "up"){
  ...
  out4 <- out
  save(out4,file=paste0(DATADIR,"33_DRE_DE_GOEnrichment_byGenomicfeatures.RData"))
