...
if(F){
  ...
  save(pllist,file=paste0(DATADIR,"33_EditingMetagenePlots_IntronExonUTRs.RData"))
  ...
  pllist <- myf_GeneratePlotDf(uq)
  save(pllist,file=paste0(DATADIR,"33_EditingMetagenePlots_IntronExonUTRs_D7vsControl.RData"))
  
  uq <- res[res$comparison=="D14_control"]
  uq$sample_name <- "control"
  uq[uq$logFC>0]$sample_name <- "D14"
  pllist <- myf_GeneratePlotDf(uq)
  save(pllist,file=paste0(DATADIR,"33_EditingMetagenePlots_IntronExonUTRs_D14vsControl.RData"))
