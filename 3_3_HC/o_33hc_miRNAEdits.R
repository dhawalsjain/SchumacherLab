...
if(F){
  ...
  editedmiRs <- o
  save(editedmiRs, file=paste0(DATADIR,"33hc_EditedmiRNAs.RData"))
}
...
if(F){
  ...
  miRgain <- p
  miRloss <- q
  save(miRgain,miRloss,file=paste0(DATADIR,"33hc_miRbinding_GainLoss_Timecourse.RData"))
  ...
  miRLGDF <- out
  save(miRLGDF,file=paste0(DATADIR,"33hc_miR_lossGain_dataframe.RData"))
...
if(F){
  ...
  save(td,file=paste0(DATADIR,"33hc_PlotDF_miRNA_mRNA_correlations_SeedBased_ALL.RData"))
  ...
  save(miRLGDF,file=paste0(DATADIR,"33hc_miR_lossGain_dataframe_New.RData"))
  ...
  td$id <- NULL
  save(td,file=paste0(DATADIR,"33hc_PlotDF_miRNA_mRNA_correlations_SeedBased.RData"))
...
if(F){
  load(paste0(DATADIR,"33hc_PlotDF_miRNA_mRNA_correlations_SeedBased_ALL.RData"))
  load(paste0(DATADIR,"33hc_miRSeedOverlaps_filtermiRNAs.RData"))
  load(paste0(DATADIR,"genes.RData"))
  ...
  td <- merge(td1,td2,by="id")
  td$id <- NULL
  save(td,file=paste0(DATADIR,"33hc_PlotDF_miRNA_mRNA_correlations_SeedBased_forTarBaseValTargets.RData"))
...
if(F){
  ...
  sq$id <- NULL
  save(sq,file=paste0(DATADIR,"33hc_PlotDF_EditedMiRNA_CorrBn_valTarsNeditingLevels.RData"))
...
if(F){
  ...
  save(consv,file=paste0(DATADIR,"33hc_ConservedMiRTargets_EnrichAlongEdits.RData"))
...
if(F){
  ...
  load(paste0(DATADIR,"DEExpression_DESeq2.RData"))
  load(paste0(DATADIR,"genes.RData"))
  load(paste0(DATADIR,"33hc_ConservedMiRTargets_EnrichAlongEdits.RData"))
  miRLGDF <- consv
  rm(consv)
  ...
  ## miRNA-mRNA influenced pairs, based on our data 
  td <- td[abs(td$cor)>0.4 & td$validity>0,]
  td$id <- NULL
  td <- data.table(td)
  td <- td[,miRs:=paste0(mir,collapse = ","),by=list(gene,idx,ref)]
  td <- as.data.frame(td)
  ...
  td <- merge(td1,td2,by="id")
  td$id <- NULL
  save(td,file=paste0(DATADIR,"33hc_PlotDF_miRNA_mRNA_correlations_PredTargets.RData"))
