...
if(F){
  ...
  save(Tx,altTX,g,gr,file = paste0(DATADIR,"33hc_miRpreds_intermediateTmp.RData"))
...
if(F){
  load(paste0(DATADIR,"33hc_miRpreds_intermediateTmp.RData"))
  width(Tx)==width(altTX)
  nn <- names(Tx)
  ...
  save(mirSeedOlp,file=paste0(DATADIR,"33hc_miRSeedOverlaps_filtermiRNAs.RData"))
...
if(F){
  ...
  for(f in levels(as.factor(outr$snpeff_uq))){
    ...
    names(seq) <- o$id
    writeXStringSet(seq,file=paste0(DATADIR,"33hc_Edit_25bpContext_",f,".fa"))
    rm(o,seq,f)
  }
  ...
  names(seq) <- outr$id
  writeXStringSet(seq,file=paste0(DATADIR,"33hc_Edit_25bpContext_all.fa"))
  
  h <-as.character(hlpr[hlpr$control>0,]$id) 
  h <- subset(h,h%in%names(seq))
  writeXStringSet(seq[h],file=paste0(DATADIR,"33hc_Edit_25bpContext_Control.fa"))
  
  h <-as.character(hlpr[hlpr$D7>0,]$id) 
  h <- subset(h,h%in%names(seq))
  writeXStringSet(seq[h],file=paste0(DATADIR,"33hc_Edit_25bpContext_D7.fa"))
  
  h <-as.character(hlpr[hlpr$D14>0,]$id) 
  h <- subset(h,h%in%names(seq))
  writeXStringSet(seq[h],file=paste0(DATADIR,"33hc_Edit_25bpContext_D14.fa"))
...
if(F){
  ...
  save(altTX,Tx,g,gr,paste0(DATADIR,"33hc_miRpreds_intermediateTmp_Clustered.RData"))
...
if(F){
  ...
  save(Cntr,Exp,file=paste0(DATADIR,"33hc_EditingClusters_GenomicSequences.RData"))
  writeXStringSet(Cntr,file=paste0(DATADIR,"33hc_DRE_Clustered_RefSeqs_Genomic.fa"))
  writeXStringSet(Exp,file=paste0(DATADIR,"33hc_DRE_Clustered_AltSeqs_Genomic.fa"))
