...
if(F){
  ...
  save(hc_edits,imputed_edits,file=paste0(DATADIR,"33hc_EditingLevelReport.RData"))
...
if(F){
  ...
  load(paste0(DATADIR,"33hc_EditingLevelReport.RData"))
  ...
  save(hc_edits_anno,imputed_edits,hlpr,file=paste0(DATADIR,"33hc_EditingLevelReport_Anno.RData"))
...
if(F){
  ...
  write.table(m[1:1000,],file=paste0(RAWDIR,"33hc_HC_EditingEvents1.bed"),quote = F,sep = "\t",row.names = F,col.names = F)
  write.table(m[1001:2000,],file=paste0(RAWDIR,"33hc_HC_EditingEvents2.bed"),quote = F,sep = "\t",row.names = F,col.names = F)
  write.table(m[2001:nrow(m),],file=paste0(RAWDIR,"33hc_HC_EditingEvents3.bed"),quote = F,sep = "\t",row.names = F,col.names = F)
  m$x <- paste0(m$seqnames,":",m$start,"-",m$end)
  write.table(m$x,file=paste0(RAWDIR,"33hc_HC_EditingEvents.liftOver"),quote = F,sep = "\t",row.names = F,col.names = F)
  
  
  ## 
  m <- read.delim(paste0(RAWDIR,"33hc_HC_EditingEvents_PhastCons60Way.txt"),header=F,comment.char = "#",stringsAsFactors = F)
  m$V1 <- gsub("chr","",m$V1)
...
if(F){
  ...
  a <- read.delim(paste0(RAWDIR,"33hc_HC_EditingEvents.liftover"),header=F,stringsAsFactors = F)
  f <- read.delim(paste0(RAWDIR,"33hc_liftoverFailure_mm102hg19.txt"),header=F,stringsAsFactors = F,comment.char = "#")
  b <- read.delim(paste0(RAWDIR,"33hc_liftover_mm102hg19.txt.bed"),header=F,stringsAsFactors = F)
...
if(F){
  ...
  cHMMEnrich <- pl
  save(cHMMEnrich,file=paste0(DATADIR,"33hc_cHMM_pValEnrichment.RData"))
...
if(F){
  ...
  save(rd,file=paste0(DATADIR,"33hc_Distance2Wt1Peaks_plotDF.RData"))
...
if(F){
  ...
  eDepth <- z
  save(oDepth,eDepth,file=paste0(DATADIR,"33hc_readDepth_EditLevels.RData"))
...
if(T){
  ...
  SPN <- z
  save(SPN,file=paste0(DATADIR,"33hc_Spreadsheet_HCEditsWithCompleteAnno.RData"))
...
if(T){
  ...
  SPNF <- z
  save(SPNF,file=paste0(DATADIR,"33hc_Spreadsheet_HCEditsWithCompleteAnnoFull.RData"))
  
}

### write spreadsheets
if(T){
  load(paste0(DATADIR,"33hc_Spreadsheet_HCEditsWithCompleteAnnoFull.RData"))
...
if(F){
  ...
  save(out,file=paste0(DATADIR,file="33hc_GOEnrichment_AllEditing.RData"))
...
if(F){
  ...
  df1$condition <- gsub("_R[123]","",df1$sample_name)
  df1$condition <- factor(df1$condition,levels=c("control","D7","D14"))
  
  dodge <- position_dodge(width = 0.75)
  comparisons <- list(c("control","D7"),c("control","D14"))
  
  q <- ggplot(df1, aes(x=anno,y=H,fill=condition,alpha=0.2)) + geom_violin(position=dodge) + theme_bw()+
    geom_boxplot(width=0.05, outlier.size = NULL,position=dodge)+
    facet_wrap(~anno)+
  ...
  pdf(paste0(FIGDIR,"EditingLevelsByAnnotations_LOI.pdf"),width = 12,height = 4)
  q
  dev.off()
