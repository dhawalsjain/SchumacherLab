...
if(!file.exists(paste0(DATADIR,"miRpreds_intermediateTmp.RData"))){
  ...
  save(Tx,altTX,g,gr,file = paste0(DATADIR,"miRpreds_intermediateTmp.RData"))
  
  s1 <- Tx[trim(resize(g,201,"center"))]
  s2 <- altTX[trim(resize(g,201,"center"))]
  names(s2) <- paste0(names(s2),"_alt")
  #writeXStringSet(s1,file=paste0(DATADIR,"referenceTrim.fa"))
  #writeXStringSet(s2,file=paste0(DATADIR,"alteredTrim.fa"))
...
if(F){
  library(Biostrings)
  load(paste0(DATADIR,"miRpreds_intermediateTmp.RData"))
  width(Tx)==width(altTX)
  nn <- names(Tx)
  ...
  save(mirSeedOlp,file=paste0(DATADIR,"miRSeedOverlaps_filtermiRNAs.RData"))
  
  #names         mir seq             idx
  # ENSMUST00000000608_1681  miR-547-5p ref 11:55110583:A>G
...
if(F){
  ...
  for(f in levels(as.factor(outr$snpeff_uq))){
    ...
    names(seq) <- o$id
    writeXStringSet(seq,file=paste0(DATADIR,"Edit_25bpContext_",f,".fa"))
    rm(o,seq,f)
  }
  ...
  writeXStringSet(seq[h],file=paste0(DATADIR,"Edit_25bpContext_Control.fa"))
  
  h <-as.character(hlpr[hlpr$D7>0,]$id) 
  h <- subset(h,h%in%names(seq))
  writeXStringSet(seq[h],file=paste0(DATADIR,"Edit_25bpContext_D7.fa"))
  
  h <-as.character(hlpr[hlpr$D14>0,]$id) 
  h <- subset(h,h%in%names(seq))
  writeXStringSet(seq[h],file=paste0(DATADIR,"Edit_25bpContext_D14.fa"))
  
}

### Hyperedited sites -transcript sequences (mRNA)
if(!file.exists(paste0(DATADIR,"miRpreds_intermediateTmp_Clustered.RData"))){
  ...
  save(altTX,Tx,g,gr,paste0(DATADIR,"miRpreds_intermediateTmp_Clustered.RData"))
...
if(F){
  ...
  save(Cntr,Exp,file=paste0(DATADIR,"EditingClusters_GenomicSequences.RData"))
  writeXStringSet(Cntr,file=paste0(DATADIR,"DRE_Clustered_RefSeqs_Genomic.fa"))
  writeXStringSet(Exp,file=paste0(DATADIR,"DRE_Clustered_AltSeqs_Genomic.fa"))
