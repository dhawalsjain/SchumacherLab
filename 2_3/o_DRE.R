...
if(!file.exists(paste0(DATADIR,"DRE_DE_matrix.RData"))){
  ...
  save(res,file=paste0(DATADIR,"DRE_DE_matrix.RData"))
...
if(F){
  ...
  save(out,out1,file=paste0(DATADIR,"DRE_DE_GOEnrichment.RData"))
...
if(F){
  ...
  r1 <- r1[,c("idx", "logFC", "adj.P.Val","change", "comparison","expn_fdr", "expn_l2fc")] %>% unique()
  names(r1) <- c("idx", "log2FC,EditLevel", "FDR_editChange","(D7-control)", "comparison","FDR_expnChange", "log2FC,D7/control")
...
if(F){
  ...
  load(paste0(DATADIR,"DRE_DE_matrix.RData"))
  res <- res[res$adj.P.Val<0.05 & res$snpeff_uq=="3_prime_UTR",]
  out <- c()
  x <- res[res$comparison=="D7_control" & res$logFC>0,]$tracking_id %>% unique
  x <- gGenes[gGenes$gene_id%in%x,]$ucscID
  y <- res[res$comparison=="D7_control" & res$logFC<0,]$tracking_id %>% unique
  y <- gGenes[gGenes$gene_id%in%y,]$ucscID
  x1 <- res[res$comparison=="D14_control" & res$logFC>0,]$tracking_id %>% unique
  x1 <- gGenes[gGenes$gene_id%in%x1,]$ucscID
  y1 <- res[res$comparison=="D14_control" & res$logFC<0,]$tracking_id %>% unique
  y1 <- gGenes[gGenes$gene_id%in%y1,]$ucscID
  
  out <- rbind(
    cbind(mm9_GOStats.enrichment(x,p.val = 0.1,universe = exp.trx),comparison="D7_overedited",change="all"),
    cbind(mm9_GOStats.enrichment(y,p.val = 0.1,universe = exp.trx),comparison="D7_underedited",change="all"),
    cbind(mm9_GOStats.enrichment(x1,p.val = 0.1,universe = exp.trx),comparison="D14_overedited",change="all"),
    cbind(mm9_GOStats.enrichment(y1,p.val = 0.1,universe = exp.trx),comparison="D14_underedited",change="all")
  )
  out2 <- out  
  save(out2,file=paste0(DATADIR,"DRE_DE_GOEnrichment_3UTR.RData"))
  
}


#### GO enrichments for introns
if(F){
  load(paste0(DATADIR,"DEExpression_DESeq2.RData"))
  ggg <- out0[[1]][,c("id","geneStart","geneEnd")]
  ggg$length <- abs(ggg$geneStart-ggg$geneEnd)
  ...
  load(paste0(DATADIR,"DRE_DE_matrix.RData"))
  res <- res[res$adj.P.Val<0.05 & res$snpeff_uq=="intron",]
  out <- c()
  x <- res[res$comparison=="D7_control" & res$logFC>0,]$tracking_id %>% unique
  x <- gGenes[gGenes$gene_id%in%x,]$ucscID
  y <- res[res$comparison=="D7_control" & res$logFC<0,]$tracking_id %>% unique
  ...
  save(out3,file=paste0(DATADIR,"DRE_DE_GOEnrichment_intron.RData"))
...
if(F){
  ...
  out <- c()
  for(feat in levels(as.factor(res$snpeff_uq)) ){
    for(f in c("up","dn")){
      cat(feat,"\t",f,"..\n")
      if(f == "up"){
        x <- res[res$comparison=="D7_control" & res$logFC>0 ,]$tracking_id %>% unique
  ...
  out4 <- out
  save(out4,file=paste0(DATADIR,"DRE_DE_GOEnrichment_byGenomicfeatures.RData"))
