...
```{r, p1, echo=T,fig.width=8,fig.height=6}
 datatable(libs, extensions = 'Buttons',options = list(searching = TRUE,pageLength = 10,dom = 'Bfrtip', 
                   buttons = c('excel', "csv")))
```
...
- Each editing site for a given sample was filtered using the criteria outlined on the figure. 
    ...
    * Sites with 100% editing were ommitted (as potential somatic mutation at the DNA level)
    * Edits present in 2/3 replicates are retained as high confidence sites
    * Missing editing level values for each condition (i.e. control, D7, D14) were imputed using mice (ref-5) package in R

![Workflow used to identify editing events](A:/work/Kreidberg lab/Valerie/PAPER/FIG/workflow.png)
...
```{r, p29, echo=T,fig.width=15,fig.height=8}
...
```{r, p30, echo=T,fig.width=15,fig.height=8}
...
```{r, p31, echo=T,fig.width=15,fig.height=8}
...
```{r, p32, echo=T,fig.width=15,fig.height=8}
...
```{r, p33, echo=T,fig.width=15,fig.height=8}
...
- Please note: out of 4109 combined high-confidence editing events across Control,D7,D14 samples, only 1710 found stringent chromatin state annotations. The 1710 events are displayed in the heatmap
...
```{r, p34.1, echo=T,fig.width=6,fig.height=5}
 load(file=paste0(DATADIR,"cHMM_pValEnrichment.RData"))
...
![Motif analyses](A:/work/Kreidberg lab/Valerie/PAPER/FIG/motifs_2by3.png)
...
![Motif analyses](A:/work/Kreidberg lab/Valerie/PAPER/FIG/motifs_2by3_GenomicAnno.png)

- Instead, I did the same analysis of findng motif, but without segragating the sequences into control, D7 or D14 groups. 
- In total, 4106 sequences (corresponding to 4109 editing sites collectively observed across control, D7 and D14) were used for predicting sequences using MEME suite. 
- Following motif was found in 1316/4106 sites
- Please note: we can report this motif in the paper, which we also find in our control samples (above. Also, it is present in the range of 22%-33% across the conditions. Please refer control_motif in above plot)

![Motif analyses](A:/work/Kreidberg lab/Valerie/PAPER/FIG/motifs_ALLSeq_2by3.png)
...
```{r, p37, echo=T,fig.width=12,fig.height=5}
  load(paste0(DATADIR,"EditingLevelReport_Anno.RData"))
...
10. Timothy L. Bailey, Mikael Bodén, Fabian A. Buske, Martin Frith, Charles E. Grant, Luca Clementi, Jingyuan Ren, Wilfred W. Li, William S. Noble, "MEME SUITE: tools for motif discovery and searching", Nucleic Acids Research, 37:W202-W208, 2009.
