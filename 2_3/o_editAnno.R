...
###############################################################################################
if(!file.exists( paste0(DATADIR,"EditingLevelReport.RData")  )){
  ...
  z1 <- impute.data(z[,grep("D7",names(z))]) 
  z2 <- impute.data(z[,grep("D14",names(z))]) 
  z3 <- impute.data(z[,grep("control",names(z))]) 
  z <- rbind(z1,z2,z3)
  z <- reshape2::dcast(z,id~variable,value.var = "value")
  rownames(z) <- z$id
  z$id <- NULL
  z[is.na(z)] <- 0
  rm(z1,z2,z3)
  ...
  hc_edits <- o
  imputed_edits <- z
  save(hc_edits,imputed_edits,file=paste0(DATADIR,"EditingLevelReport.RData"))
  
  rm(o,z,hc_edits,imputed_edits,db,alu,mm1,regions)
}

###############################################################################################
### Annotations for clusters, uniqueness etc 
###############################################################################################
if(!file.exists( paste0(DATADIR,"EditingLevelReport_Anno.RData")  )){
  ...
  save(hc_edits_anno,imputed_edits,hlpr,file=paste0(DATADIR,"EditingLevelReport_Anno.RData"))
...
if(F){
  ...
  m$start <- m$start-1
  m <-m[,c(2,3,5,1)]
  write.table(m,file=paste0(DATADIR,"HC_EditingEvents.bed"),quote = F,sep = "\t",row.names = F,col.names = F)
  m$x <- paste0(m$seqnames,":",m$start,"-",m$end)
  write.table(m$x,file=paste0(RAWDIR,"HC_EditingEvents.liftOver"),quote = F,sep = "\t",row.names = F,col.names = F)
  
  m <- read.delim(paste0(RAWDIR,"HC_EditingEvents_PhastCons60Way.txt"),header=F,comment.char = "#",stringsAsFactors = F)
  m$V1 <- gsub("chr","",m$V1)
...
if(F){
  ...
  a <- read.delim(paste0(RAWDIR,"HC_EditingEvents.liftover"),header=F,stringsAsFactors = F)
  f <- read.delim(paste0(RAWDIR,"liftoverFailure_mm102hg19.txt"),header=F,stringsAsFactors = F,comment.char = "#")
  b <- read.delim(paste0(RAWDIR,"liftover_mm102hg19.txt"),header=F,stringsAsFactors = F)
...
if(F){
  ...
  cHMMEnrich <- pl
  save(cHMMEnrich,file=paste0(DATADIR,"cHMM_pValEnrichment.RData"))
...
if(F){
  ...
  save(rd,file=paste0(DATADIR,"Distance2Wt1Peaks_plotDF.RData"))
...
if(F){
  ...
  eDepth <- z
  save(oDepth,eDepth,file=paste0(DATADIR,"readDepth_EditLevels.RData"))

}

### coplete editing annotation spreadsheet (merged replicates)
if(T){
  ...
  SPN <- z
  save(SPN,file=paste0(DATADIR,"Spreadsheet_HCEditsWithCompleteAnno.RData"))
...
if(T){
  ...
  SPNF <- z
  save(SPNF,file=paste0(DATADIR,"Spreadsheet_HCEditsWithCompleteAnnoFull.RData"))
...
if(F){
  ...
  save(out,file=paste0(DATADIR,file="GOEnrichment_AllEditing.RData"))
